...
float aoFactor(vec3 pos, vec3 pos_2, vec3 dir) {
    const vec3 v = pos - pos_2 - 1e-5;
    return
...
void main() {
    ...
    const int deps_len = 8;
    const vec2 size = textureSize(depth);
    ...
    for (int i = 0; i < locations.length(); i++) {
        ...
        const float deps[] = float[deps_len](
            getDepth(iuv + off),
            getDepth(iuv + ivec2(off.x, 0)),
            getDepth(iuv + ivec2(0, off.y)),
        ...
        for (int j = 0; j < deps.length(); j++) {
            ...
            pos_2.xyz /= pos_2.w;
            ao += aoFactor(pos.xyz, pos_2.xyz, n);
        }
    }
