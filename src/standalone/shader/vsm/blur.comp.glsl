...
layout(set = 0, binding = 1, rgba32f) writeonly uniform image2D dest;
...
void main() {
    ...
    if (gid.y < kernel.length() / 2 || gid.y + kernel.length() / 2 >= bounds.y) {
    ...
    for (int i = -kernel.length() / 2; i <= kernel.length() / 2; i++)
        sum += imageLoad(source, ivec2(gid.x, gid.y + i)).rg * kernel[k++];
